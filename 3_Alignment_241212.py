...
def inverse_alignment(
    image_path,            #加工画像
    scale_factor,          #推定された拡大率
    rotation_angle,        #推定された回転率
    full_image_size,       #元画像input.jpgのサイズ (w, h)
    paste_position,        #元画像で切り出した左上の座標 (x, y)
    output_path            #保存パス
):
    #加工画像を開く
    ...
    #元画像サイズのキャンバスを作成
    canvas = Image.new("RGB", full_image_size, (0, 0, 0))

    #元画像の切り出し位置に貼り付ける
    canvas.paste(img, paste_position)

    #保存
    save(canvas, output_path, "final_aligned.jpg"),
    print(f"保存完了: {output_path}")


# --- 逆変換の実行 ---
picture_path = "./picture/"
original_img = Image.open("picture/input.jpg")  # パス指定して開く
original_size = original_img.size 
xy = (700, 900)

inverse_alignment(
    image_path=data_path + input_name_2,
